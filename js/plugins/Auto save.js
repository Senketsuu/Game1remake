#==============================================================================
#
# ?\ Yanfly Engine Ace - Ace Save Engine v1.03 (1.0)
...
module YEA
  module SAVE
    ...
    SLOT_NAME = "Fichier %s"  # How the file slots will be named.
    
    # These are the icons
    SAVE_ICON  = 988       # Icon used to indicate a save is present.
    EMPTY_ICON = 989       # Icon used to indicate an empty file.
    ...
    ACTION_SAVE   = "Sauver"           # Text used for saving games.
    ...
    SELECT_HELP = "Sélectionnez un fichier."
    LOAD_HELP   = "Charge les données du fichier sauvegardé."
    SAVE_HELP   = "Sauvegarde la progression actuelle."
    ...
    COLUMN1_VARIABLES = [9, 7, 8]
    COLUMN2_VARIABLES = [10, 5, 6]
    FACE_DISPLAY = true
...
class Window_FileStatus < Window_Base
  ...
  def draw_save_characters(dx, dy)
    if YEA::SAVE::FACE_DISPLAY then
      ...
      for member in @header[:party].battle_members
        ...
        change_color(normal_color)
        draw_actor_face(member, dx-x/3, dy/2+line_height)
        text = member.name
        draw_text(dx-x/3, dy+x*2/3-line_height/2, dw, line_height, text, 3)
        change_color(system_color)
        text = Vocab::level_a
        draw_text(dx-x/3, dy+x*2/3-line_height*5/4, dw, line_height, text, 3)
        cx = text_size(text).width
        change_color(normal_color)
        text = member.level.group
        draw_text(dx-x/3+x/6, dy+x*2/3-line_height*5/4, dw+cx-4, line_height,
          text, 3)
        dx += dw
      end
    end
  ...
  def draw_column_data(data, dx, dy, dw)
    ...
    for variable_id in data
      ...
      draw_text(dx, dy+line_height*7/3, dw, line_height, name, 0)
      value = @header[:variables][variable_id].group
      change_color(normal_color)
      draw_text(dx, dy+line_height*7/3, dw, line_height, value, 2)
      dy += line_height
      end
    end
    return if YEA::SAVE::FACE_DISPLAY
...
module YEA
  module SAVE
    ...
    CONFIRM_SAVE = "Sauvegarder par dessus ce fichier ?"
    CONFIRM_LOAD = "Charger ce fichier ?"
  end
end
